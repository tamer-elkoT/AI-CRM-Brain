...
export default function Auth() {
  ...
  const { login, signup, verifyOtp, googleLogin, isAuthenticated } = useAuth();
  ...
  const [role, setRole] = useState<'Sales' | 'Client'>('Sales');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        // Step 1: Create account → returns pending response (no token)
  ...
  const handleOtpVerify = async (otp: string) => {
    await verifyOtp({ phone_number: pendingPhone, otp_code: otp });
    navigate('/onboarding');
  };
              ...
              <form className="space-y-4" onSubmit={handleSubmit}>
                {!isLogin && (
                  <>
                    {/* ── Role Selector ── */}
                    <div>
                      <label className="block font-label-sm text-label-sm text-on-surface mb-2">I am a...</label>
                      <div className="flex gap-3">
                        <button
                          type="button"
                          onClick={() => setRole('Sales')}
                          className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-lg border-2 transition-all duration-200 font-label-md text-label-md ${
                            role === 'Sales'
                              ? 'border-secondary bg-secondary/10 text-secondary shadow-sm'
                              : 'border-outline-variant bg-surface text-on-surface-variant hover:border-secondary/40'
                          }`}
                        >
                    ...
                    {/* ── Phone Number ── */}
                    <div>
                      <label className="block font-label-sm text-label-sm text-on-surface mb-1" htmlFor="auth-phone">
                        WhatsApp Phone Number
                      </label>
                      <div className="relative">
                        <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-[#25D366] text-xl">smartphone</span>
                        <input
                      ...
                      <p className="mt-1.5 font-body-sm text-body-sm text-on-surface-variant flex items-start gap-1">
                        <span className="material-symbols-outlined text-[14px] mt-0.5 text-[#25D366]">info</span>
                        Please ensure this number is registered with WhatsApp to receive your verification code and deal alerts.
                      </p>
                    </div>
                  </>
                )}
